import {
  Icon_default
} from "./chunk-V6KURXND.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  snippet,
  spread_props,
  wrap_snippet
} from "./chunk-PNRPTIB5.js";
import "./chunk-U7P2NEEE.js";
import {
  append,
  comment
} from "./chunk-FD57FX2G.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  noop,
  pop,
  push
} from "./chunk-2ZDPGQTT.js";
import "./chunk-XWATFG4W.js";
import "./chunk-HNWPC2PS.js";
import "./chunk-DZ7PW52E.js";
import "./chunk-OHYQYV5R.js";
import "./chunk-G3PMV62Z.js";

// node_modules/@lucide/svelte/dist/icons/locate.svelte
Locate[FILENAME] = "node_modules/@lucide/svelte/dist/icons/locate.svelte";
function Locate($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, Locate);
  let props = rest_props($$props, ["$$slots", "$$events", "$$legacy"], "props");
  const iconNode = [
    ["line", { "x1": "2", "x2": "5", "y1": "12", "y2": "12" }],
    ["line", { "x1": "19", "x2": "22", "y1": "12", "y2": "12" }],
    ["line", { "x1": "12", "x2": "12", "y1": "2", "y2": "5" }],
    ["line", { "x1": "12", "x2": "12", "y1": "19", "y2": "22" }],
    ["circle", { "cx": "12", "cy": "12", "r": "7" }]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => (
      /**
      * @component @name Locate
      * @description Lucide SVG icon component, renders SVG Element with children.
      *
      * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/locate
      * @see https://lucide.dev/guide/packages/lucide-svelte - Documentation
      *
      * @param {Object} props - Lucide icons props and any valid SVG attribute
      * @returns {FunctionalComponent} Svelte component
      *
      */
      Icon_default(node, spread_props({ name: "locate" }, () => props, {
        get iconNode() {
          return iconNode;
        },
        children: wrap_snippet(Locate, ($$anchor2, $$slotProps) => {
          var fragment_1 = comment();
          var node_1 = first_child(fragment_1);
          add_svelte_meta(() => snippet(node_1, () => $$props.children ?? noop), "render", Locate, 62, 2);
          append($$anchor2, fragment_1);
        }),
        $$slots: { default: true }
      }))
    ),
    "component",
    Locate,
    61,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  Locate = hmr(Locate);
  import.meta.hot.accept((module) => {
    Locate[HMR].update(module.default);
  });
}
var locate_default = Locate;
export {
  locate_default as default
};
//# sourceMappingURL=@lucide_svelte_icons_locate.js.map
